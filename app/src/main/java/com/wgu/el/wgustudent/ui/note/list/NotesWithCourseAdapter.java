package com.wgu.el.wgustudent.ui.note.list;

import android.content.Context;
import android.support.v7.widget.RecyclerView;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.wgu.el.wgustudent.R;
import com.wgu.el.wgustudent.entity.NoteWithCourse;

import java.util.List;

// TODO show associated course in list, not implemented for this assignment
public class NotesWithCourseAdapter extends RecyclerView.Adapter<NotesListViewHolder>  {


    private final Context context;
    private List<NoteWithCourse> items;
    private View.OnClickListener deleteClickListener;
    private View.OnClickListener itemClickListener;

    public NotesWithCourseAdapter(List<NoteWithCourse> items, Context context, View.OnClickListener itemClickListener, View.OnClickListener  deleteClickListener) {
        this.items = items;
        this.context = context;
        this.itemClickListener = itemClickListener;
        this.deleteClickListener = deleteClickListener;
    }

    @Override
    public NotesListViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.item_notes_list, parent, false);
        return new NotesListViewHolder(v);
    }


    @Override
    public void onBindViewHolder(NotesListViewHolder holder, int position) {
        NoteWithCourse item = items.get(position);

        holder.noteNameTv.setText(item.noteName);
        holder.noteAssociatedCourseTv.setText(item.courseName);
        holder.itemView.setTag(item);
        holder.deleteButton.setTag(item);
        holder.deleteButton.setOnClickListener(deleteClickListener);
        holder.itemView.setOnClickListener(itemClickListener);

    }

    @Override
    public int getItemCount() {
        return items.size();
    }

    void setItems(List<NoteWithCourse> notes) {
        this.items = notes;
        notifyDataSetChanged();
    }

}
